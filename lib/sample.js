...
class Sample {
    static async addFile() {
        ...
        setTimeout(async () => {
            let resp = await fetch(`http://localhost:26657/tx?hash=0x${receipt.txhash}&prove=true`);
            const o = await resp.json();
            console.log(o, receipt);
            let key = "bafyreiapkje327erp4u736b7juu27jd3isuizupma775cpw6p4yvg7plyi";
            ...
            expobj.path.forEach((inner) => {
                const abiLeadOp = ethers_1.ethers.utils.defaultAbiCoder.encode(["tuple(HashOp,bytes,bytes)"], [inner.hash, inner.prefix, inner.suffix]);
