...
class Test {
    static async uploadFile() {
        ...
        const msg = tx_1.MsgFile.fromPartial({
            creator: address,
        ...
        provider.tmclient.subscribeTx(query).addListener({
            next: async (log) => {
                console.log(log);
                const h = tx_1.MsgFileMetadataResponse.decode(log.result.data);
                // const rd = new Reader(log.result.data)
                console.log(tx_1.MsgFileMetadataResponse.toJSON(h));
                // rd.uint32()
                // rd.string()
                // const c = rd.skip(4)
        ...
        const ss = await provider.ancon.signAndBroadcast([provider.ancon.msgFile(msg)], {
            fee: {
                amount: [
                    {
                        denom: 'token',
