...
class AnconClient {
    ...
    async sign(accountNumber, address, chainId, sequence, fee, encoded, signer, keyringAccount) {
        ...
        const bodyBytes = module_1.registry.encode(txBodyEncodeObject);
        const signDoc = tx_3.SignDoc.fromPartial({
            chainId,
            accountNumber,
            bodyBytes,
    ...
    async create(accountName, passphrase, mnemonic) {
        ...
        const cosmosAccount = await this.getEthAccountInfo("0xB9F6914A7415F9AD867A9C537471A46DFA852BAE");
