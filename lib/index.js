...
class AnconClient {
    ...
    async sign(accountNumber, address, chainId, sequence, fee, encoded, signer) {
        ...
        const txRawBytes = Uint8Array.from(tx_2.TxRaw.encode(txRaw).finish());
        const txBytesHex = ethers_1.ethers.utils.hexlify(txRawBytes);
        return txBytesHex;
    }
    async create(accountName, passphrase, mnemonic) {
        ...
        if (!this.signer) {
            ...
            this.ethersclient = ethers_1.ethers.Wallet.fromMnemonic(keystore.mnemonic);
            this.ethersclient.connect(rpc);
            signer = await proto_signing_1.DirectSecp256k1HdWallet.fromMnemonic(keystore.mnemonic, {
                prefix: 'ethm',
            });
        }
        this.offlineSigner = await stargate_1.SigningStargateClient.offline(signer, {
        ...
        const defaultAccount = this.account[0];
